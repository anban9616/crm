				...
				<c:forEach items="${requestScope.list }" var="item" varStatus="status">
					<tr class="text-c">
						...
						<td class="td-manage">
							...
							<a title="放入客户池" href="javascript:;" onclick="customer_release(this,${item.id})" class="ml-5" style="text-decoration: none">
								<i class="Hui-iconfont">&#xe60e;</i>
							</a>
...
/*客户-释放*/
function customer_release(obj,id){
	layer.confirm('确认要放入客户池吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '${path}/customer/releaseOne?id='+id,
			dataType: 'json',
			success: function(data){
				$(obj).parents("tr").remove();
				layer.msg('已放入客户池!',{icon:1,time:1000});
			},
			error:function(data) {
				console.log(data);
			},
