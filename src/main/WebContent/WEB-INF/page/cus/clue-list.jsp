			...
			<tbody>
				<c:forEach items="${requestScope.list }" var="item" varStatus="status">
					<tr class="text-c">
						...
						<td class="td-manage">
							<a title="编辑" href="javascript:;" onclick="clue_edit('线索编辑','${path}/clue/edit?id=${item.id }','800','500')" class="ml-5"
								style="text-decoration: none">
								<i class="Hui-iconfont">&#xe6df;</i>
							</a> 
							<a title="放入线索池" href="javascript:;" onclick="clue_release(this,${item.id})" class="ml-5" style="text-decoration: none">
								<i class="Hui-iconfont">&#xe60e;</i>
							</a>
							<a title="转化" href="javascript:;" onclick="clue_transform(this,${item.id})" class="ml-5" style="text-decoration: none">
								<i class="Hui-iconfont">&#xe6ab;</i>
							</a>
...
/*线索-释放*/
function clue_release(obj,id){
	layer.confirm('确认要放入线索池吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '${path}/clue/releaseOne?id='+id,
			dataType: 'json',
			success: function(data){
				$(obj).parents("tr").remove();
				layer.msg('已放入线索池!',{icon:1,time:1000});
			},
			error:function(data) {
				console.log(data);
			},
...
/*线索-转化*/
function clue_transform(obj,id){
	layer.confirm('确认要转化线索吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '${path}/clue/transformOne?id='+id,
			dataType: 'json',
			success: function(data){
				$(obj).parents("tr").remove();
				layer.msg('已转化线索!',{icon:1,time:1000});
			},
			error:function(data) {
				console.log(data);
			},
