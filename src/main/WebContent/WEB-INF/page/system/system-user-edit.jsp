...
<title>编辑用户 - 用户管理 - H-ui.admin v3.1</title>
</head>
<body>
	<article class="page-container">
		<form class="form form-horizontal" id="form-admin-edit">
			<input name="id" type="hidden" value="${user.id }">
			<input id="sex"	type="hidden" value="${user.sex }">
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" value='<fmt:formatDate value="${user.birth }"/>' placeholder="" id="birth" name="birthStr">
				</div>
			</div>
			<div class="row cl">
					...
					<span class="select-box" style="width: 150px;">
						<select class="select" name="roleId" size="1">
							<c:forEach var="item" items="${requestScope.roleList }">
								<option value="${item.id }" ${(user.roleId eq item.id)?'selected':'' }>${item.name }</option>
							</c:forEach>
						</select>
					</span>
				</div>
				...
				<div class="formControls col-xs-8 col-sm-9">
					<span class="select-box" style="width: 150px;">
						<select class="select" name="deptId" size="1">
							<c:forEach var="item" items="${requestScope.deptList }">
								<option value="${item.id }" ${(user.deptId eq item.id)?'selected':'' }>${item.name }</option>
							</c:forEach>
						</select>
					</span>
				</div>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9 skin-minimal">
					<div class="radio-box">
						<input type="radio" id="status-0" name="status" value="0"> <label for="status-0">启用</label>
					</div>
					<div class="radio-box">
						<input type="radio" id="status-1" name="status" value="1"> <label for="status-1">停用</label>
					</div>
				</div>
			</div>
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" value="${user.remark }" placeholder="" id="remark" name="remark">
				</div>
			</div>
			<div class="row cl">
	...
	<script type="text/javascript">
		$(function() {
			...
			function check(){
				var sex = $("#sex").val();
				$(":radio[name='sex']").eq(sex).attr("checked","checked");
				var status = $("#status").val();
				$(":radio[name='status']").eq(status).attr("checked","checked");
			}
			check();
			//设置checked的函数需要在以下函数执行前面，原因不明
